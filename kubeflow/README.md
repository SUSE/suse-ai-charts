...
global:
  ...
  # <name>-<namespace>.models.dev.example.com. Omit this and it silently falls back
  # to example.com.
  kserveDomain: "models.dev.example.com"
...
global:
  kserveDomain: "models.example.com"  # base domain for inference hosts + wildcard cert. It does
                                     # NOT have to match the dashboard hostname below — here the
                                     # dashboard lives at kubeflow.example.com while inference
                                     # serves under *.models.example.com.
                                     # Prefer a scoped subdomain (e.g. models.example.com)
...
kubeflow-istio-resources:
  hostname: "kubeflow.example.com"   # required: the dashboard host. With kserveExternalHttps the
                                     ...
                                     # separate from the *.models.example.com inference wildcard. Using
...
| `global.kserveDomain` | `""` | Base domain for the KServe wildcard cert + gateway server (`*.<global.kserveDomain>`). The **recommended single place** to set the inference domain: when set it overrides and auto-wires `kserve.ingressDomain` and `knativeServing.domain` — set it here, not on the subcharts. Empty = the whole stack stays on `example.com` (the subchart-local `knativeServing.domain` / `kserve.ingressDomain` defaults). This is the **enforced single source of truth**: to override the domain you must set it here — setting the subchart keys alone (without `global.kserveDomain`) fails fast, because they feed the Knative/KServe configs but not the dedicated gateway's hosts, which would diverge and break inference readiness. **Required** when `global.kserveExternalHttps` is `true` |
...
| Key | Default | Description |
|-----|---------|-------------|
| `global.kserveDomain` | `""` | Root domain for KServe InferenceService ingress — the **enforced single source of truth**. When set, it overrides and auto-wires the legacy `knativeServing.domain` / `kserve.ingressDomain`. Setting those subchart keys alone (without `global.kserveDomain`) fails fast — they don't reach the dedicated gateway's hosts, so the chart rejects the divergence. Empty = the stack stays on `example.com`. With `global.kserveExternalHttps: true` it is also the wildcard cert/gateway base |
...
The KServe/Knative inference domain now has one home under
**`global.kserveDomain`** — the enforced single source of truth. Set it there and it
overrides and auto-wires the subchart keys. Setting the subchart keys alone
(`knativeServing.domain` / `kserve.ingressDomain`) without `global.kserveDomain` now
fails fast: they feed the Knative/KServe configs but not the dedicated
`kserve-ingress-gateway`'s hosts, so the chart rejects the divergence rather than
silently stranding inference at `READY=Unknown`.
...
> **⚠ BREAKING for installs that set `knativeServing.domain` or `kserve.ingressDomain`.**
> If you previously set either subchart key (or `_kserveAccess.domain`) to a custom value
> **without** `global.kserveDomain`, `helm upgrade` now **hard-fails** at render time with a
> `DIVERGENCE:` error naming the fix. This is intentional — the old keys reach the Knative/KServe
> configs but not the dedicated `kserve-ingress-gateway`'s hosts, so the previous behaviour
> silently stranded inference at `READY=Unknown`. **Before upgrading**, add
> `global.kserveDomain: <your-domain>` set to the same value (see below). A bare `domain:` /
> `ingressDomain:` (empty or null) is also rejected, since it renders an empty Knative domain.
